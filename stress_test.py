...
def load_full_attendee_pool(session_factories, cap_per_table, log_fn):
    ...
    for backend_name in ("mysql", "sqlite"):
        ...
        any_backend = True
        try:
            rows, counts = fetch_attendee_pool(factory, cap_per_table, exclude_synthetic=True)
        ...
        log_fn(f"{backend_name.upper()}: {added} real attendees loaded ({counts['attendees']} base, {counts['kiosk']} kiosk).")
...
class EnterpriseStressTestApp(tb.Window):
    def __init__(self):
        ...
        ww, wh = max(1200, min(1600, int(sw * 0.90))), max(800, min(950, int(sh * 0.90)))
    ...
    def _configure_custom_styles(self):
        ...
        self.style.configure("TLabelframe", background=colors.get("bg"), bordercolor=self.SOFT_BORDER, borderwidth=1)
        self.style.configure("TLabelframe.Label", background=colors.get("bg"), font=("Helvetica", 10, "bold"), foreground=colors.get("info"))
    ...
    def reload_attendee_pool(self, initial=False):
        ...
        self.log("Connecting to database to load the real attendee pool for check-in testing...")
    ...
    def _load_pool_worker(self, cap):
        try:
            ...
            if pool:
                self.log(f"SUCCESS: attendee pool ready — {len(pool)} real attendees loaded and available for check-in testing.")
            else:
                self.log(
    ...
    def _refresh_pool_label(self):
        ...
        if n == 0:
            self.lbl_pool_status.config(text="Pool: 0 attendees loaded", bootstyle="danger")
        else:
            detail = ", ".join(f"{v} from {k}" for k, v in counts.items())
            self.lbl_pool_status.config(text=f"Pool: {n} real attendees ready ({detail})", bootstyle="success")
        ...
        try:
            ...
            for backend in ("mysql", "sqlite"):
                ...
                if factory:
                    ...
                    touched += 1
            self.log(f"Synthetic check-in history reset on {touched} backend(s).")
        except Exception as e:
        ...
        try:
            ...
            for backend in ("mysql", "sqlite"):
                ...
                if factory:
                    ...
                    total_a += n1
                    total_k += n2
            self.log(f"Purged {total_a} synthetic attendees + {total_k} synthetic kiosk records.")
    ...
    def _safe_int(var, default, minimum=None):
        try: val = int(var.get())
        except (tk.TclError, ValueError, TypeError): return default
        if minimum is not None and val < minimum: return default
        return val
    ...
    def _build_control_panel(self, parent):
        tb.Label(parent, text="⚡ Load Injector Control", font=("Helvetica", 16, "bold"), bootstyle=PRIMARY).pack(anchor="w", pady=(0, 15))

        targets_lf = tb.Labelframe(parent, text=" 1. Routing Targets ", padding=12)
        ...
        for label, var in [("Waitress Engine (HTTP):", self.http_var), ("Cheroot Engine (HTTPS):", self.https_var), ("Cloudflare Tunnel (optional):", self.cloudflare_var)]:
            tb.Label(targets_lf, text=label, font=("Helvetica", 9, "bold")).pack(anchor="w")
            tb.Entry(targets_lf, textvariable=var).pack(fill="x", pady=(2, 8))

        profile_lf = tb.Labelframe(parent, text=" 2. Load Profile ", padding=12)
        profile_lf.pack(fill="x", pady=(0, 12))

        tb.Label(profile_lf, text="Action Distribution:", font=("Helvetica", 9, "bold")).pack(anchor="w")
        self.action_var = tk.StringVar(value="Strict Check-ins (Scanner Simulation)")
        action_combo = tb.Combobox(profile_lf, textvariable=self.action_var, state="readonly", values=["Strict Check-ins (Scanner Simulation)", "Strict Registrations (Kiosk Simulation)", "Mixed Load (50% Check-in / 50% Reg)"])
        action_combo.pack(fill="x", pady=(2, 8))

        tb.Label(profile_lf, text="Concurrency Attack Strategy:", font=("Helvetica", 9, "bold")).pack(anchor="w")
        self.sync_var = tk.StringVar(value="Human Pacing (1-3s delays)")
        sync_combo = tb.Combobox(profile_lf, textvariable=self.sync_var, state="readonly", values=["Human Pacing (1-3s delays)", "Gradual Ramp-Up (Stress Growth)", "Synchronized Millisecond Stampede"])
        ...
        tb.Label(day_row, text="Event Days (comma-separated):", font=("Helvetica", 9, "bold")).pack(anchor="w")
        ...
        tb.Label(year_row, text="Event Year:", font=("Helvetica", 9, "bold")).pack(side="left")
        self.event_year_var = tk.IntVar(value=DEFAULT_EVENT_YEAR)
        tb.Entry(year_row, textvariable=self.event_year_var, width=8).pack(side="right")

        exec_lf = tb.Labelframe(parent, text=" 3. Execution Parameters ", padding=12)
        ...
        for label, var in [("Devices PER Target:", self.threads_var), ("Test Duration (sec):", self.duration_var), ("Ramp-Up Window (sec):", self.rampup_var), ("Request Timeout (sec):", self.timeout_var)]:
            ...
            tb.Label(row, text=label, font=("Helvetica", 9, "bold")).pack(side="left")
            tb.Entry(row, textvariable=var, width=8).pack(side="right")

        db_lf = tb.Labelframe(parent, text=" 4. Database Tools & Cleanup ", padding=12)
        db_lf.pack(fill="x", pady=(0, 12))

        self.lbl_pool_status = tb.Label(db_lf, text="Pool: loading...", font=("Helvetica", 9), wraplength=260, justify="left")
        self.lbl_pool_status.pack(anchor="w", pady=(0, 8))

        cap_row = tb.Frame(db_lf)
        cap_row.pack(fill="x", pady=(0, 8))
        tb.Label(cap_row, text="Max rows to load:", font=("Helvetica", 9, "bold")).pack(side="left")
        self.pool_cap_var = tk.IntVar(value=DEFAULT_POOL_CAP)
        tb.Entry(cap_row, textvariable=self.pool_cap_var, width=9).pack(side="right")

        self.btn_reload = tb.Button(db_lf, text="🔄 Reload Attendee Pool", bootstyle=INFO, command=lambda: self.reload_attendee_pool(initial=False))
        self.btn_reload.pack(fill="x", pady=(0, 15))

        # Synthetic Test Data Tools
        tb.Label(db_lf, text="Synthetic Test Data:", font=("Helvetica", 9, "bold")).pack(anchor="w", pady=(0, 4))
        synth_row = tb.Frame(db_lf)
        synth_row.pack(fill="x", pady=(0, 15))
        self.btn_reset_synth = tb.Button(synth_row, text="🧹 Reset Test History", bootstyle="warning-outline", command=self.on_reset_synthetic_history)
        self.btn_reset_synth.pack(side="left", fill="x", expand=True, padx=(0, 4))
        self.btn_purge_synth = tb.Button(synth_row, text="🗑️ Purge Test Users", bootstyle="danger-outline", command=self.on_purge_synthetic)
        self.btn_purge_synth.pack(side="right", fill="x", expand=True, padx=(4, 0))
        self._tip(self.btn_reset_synth, f"Clears checkin_history ONLY for attendees named '{SYNTHETIC_NAME_PREFIX} *'")
        self._tip(self.btn_purge_synth, f"Permanently DELETES attendees named '{SYNTHETIC_NAME_PREFIX} *'")

        # Global Data Tools (DANGER)
        tb.Label(db_lf, text="Global Live Data (DANGER):", font=("Helvetica", 9, "bold")).pack(anchor="w", pady=(0, 4))
        global_row = tb.Frame(db_lf)
        global_row.pack(fill="x", pady=(0, 6))
        self.btn_reset_all = tb.Button(global_row, text="☢️ Wipe ALL Check-ins", bootstyle="danger", command=self.on_reset_all_history)
        self.btn_reset_all.pack(side="left", fill="x", expand=True, padx=(0, 4))
        self.btn_clear_kiosk = tb.Button(global_row, text="☢️ Clear ALL Kiosk Reg", bootstyle="danger", command=self.on_clear_kiosk)
        self.btn_clear_kiosk.pack(side="right", fill="x", expand=True, padx=(4, 0))
        self._tip(self.btn_reset_all, "Wipes check-in history for EVERY attendee in the database.")
        self._tip(self.btn_clear_kiosk, "Deletes EVERY registration record from the Offline Kiosk table.")

        btn_row = tb.Frame(parent)
        btn_row.pack(fill="x", pady=(10, 12))
        self.btn_start = tb.Button(btn_row, text="▶ INJECT LOAD", bootstyle=SUCCESS, command=self.start_test, state="disabled")
        self.btn_start.pack(side="left", expand=True, fill="x", padx=(0, 4))
        self.btn_stop = tb.Button(btn_row, text="■ HALT", bootstyle=DANGER, command=self.stop_test, state="disabled")
        self.btn_stop.pack(side="right", expand=True, fill="x", padx=(4, 0))

        self.progress_bar = tb.Progressbar(parent, bootstyle="success-striped", value=0)
        self.progress_bar.pack(fill="x", pady=(0, 12))

        tb.Button(parent, text="⬇ Export Raw Data (CSV)", bootstyle="secondary-outline", command=self.export_csv).pack(fill="x", pady=(0, 6))
        tb.Button(parent, text="📄 Export Summary Report", bootstyle="secondary-outline", command=self.export_summary_report).pack(fill="x")
    ...
    def _build_dashboard_tab(self, parent):
        self.lbl_verdict = tb.Label(parent, text="⚪ IDLE — no requests sent yet.", font=("Helvetica", 13, "bold"))
        self.lbl_verdict.pack(fill="x", pady=(0, 10))

        meter_row = tb.Frame(parent)
        meter_row.pack(fill="x", pady=(0, 15))
        self.meter_chk = tb.Meter(meter_row, metersize=160, padding=10, amounttotal=self.meter_max, amountused=0, metertype="semi", subtext="Check-ins / sec", interactive=False, bootstyle="success", textfont=("Helvetica", 18, "bold"))
        self.meter_chk.pack(side="left", expand=True)
        self.meter_reg = tb.Meter(meter_row, metersize=160, padding=10, amounttotal=self.meter_max, amountused=0, metertype="semi", subtext="Registrations / sec", interactive=False, bootstyle="info", textfont=("Helvetica", 18, "bold"))
        self.meter_reg.pack(side="right", expand=True)

        cards_row = tb.Frame(parent)
        cards_row.pack(fill="x", pady=(0, 15))
        card_defs = [("Total Reqs", "primary"), ("Success %", "success"), ("Avg RT (ms)", "info"), ("HTTP 503", "warning"), ("User Pool", "secondary")]
        for name, style in card_defs:
            ...
            tb.Label(card, text=name, font=("Helvetica", 9, "bold"), background=self.CARD_BG, foreground="gray").pack(anchor="w")
            val_lbl = tb.Label(card, text="0", font=("Helvetica", 20, "bold"), background=self.CARD_BG, bootstyle=style)
            val_lbl.pack(anchor="w")
            self.stat_widgets[name] = val_lbl

        if MATPLOTLIB_AVAILABLE:
            fig, (self.ax_tps, self.ax_rt) = plt.subplots(2, 1, figsize=(8, 4), facecolor=self.CARD_BG)
            ...
            self.ax_rt.text(0.99, 500, " 500ms Apdex target", color="#ffbb33", fontsize=7, ha="right", va="bottom", transform=self.ax_rt.get_yaxis_transform())
            ...
            tb.Label(parent, text="(Install 'matplotlib' via pip to view Live Traffic Graph)", font="-size 10 -slant italic", foreground="#888").pack(pady=40)
    ...
    def _build_analytics_tab(self, parent):
        ...
        chk_col = tb.Labelframe(cols, text=" Check-in Performance ", padding=15)
        chk_col.pack(side="left", fill=BOTH, expand=YES, padx=(0, 8))
        reg_col = tb.Labelframe(cols, text=" Registration Performance ", padding=15)
        ...
        err_lf = tb.Labelframe(parent, text=" Error / Rejection Breakdown ", padding=15)
        ...
        for i, (key, label_text) in enumerate(err_defs):
            ...
            tb.Label(cell, text=label_text, font=("Helvetica", 9)).pack(side="left")
    ...
    def _build_grid_tab(self, parent):
        ...
        self.tree = tb.Treeview(parent, columns=columns, show="headings", bootstyle="info")
        headings = {"time": "Time", "env": "Target", "type": "Action", "status": "Status", "rt": "RT (ms)", "identifier": "Identifier Used"}
        widths = {"time": 90, "env": 120, "type": 90, "status": 80, "rt": 80, "identifier": 160}
        for col in columns:
        ...
        vsb = tb.Scrollbar(parent, orient="vertical", command=self.tree.yview, bootstyle="round")
        self.tree.configure(yscrollcommand=vsb.set)
        self.tree.pack(side="left", fill=BOTH, expand=YES)
        vsb.pack(side="right", fill="y")

    def _build_log_tab(self, parent):
        self.log_txt = tb.Text(parent, wrap="word", state="disabled", font=("Consolas", 10), background="#1e1e1e", foreground="#d4d4d4", borderwidth=0)
        vsb = tb.Scrollbar(parent, orient="vertical", command=self.log_txt.yview, bootstyle="round")
    ...
    def start_test(self):
        ...
        raw_targets = [("Waitress", self.http_var.get().strip()), ("Cheroot", self.https_var.get().strip()), ("Cloudflare", self.cloudflare_var.get().strip())]
        targets = [(name, url) for name, url in raw_targets if url]
        if not targets:
            self.log("ERROR: no routing targets provided. Fill in at least one target URL.")
            return
        bad = [url for _, url in targets if not (url.startswith("http://") or url.startswith("https://"))]
        if bad:
            self.log(f"WARNING: target(s) missing http(s):// scheme: {', '.join(bad)} — requests will likely fail.")
        ...
        try:
            ...
            self.log("ERROR: Devices / Duration / Ramp-Up / Timeout must all be valid numbers.")
            return
        ...
        if ramp_up_sec >= duration:
            self.log(f"WARNING: ramp-up ({ramp_up_sec}s) is >= duration ({duration}s)")

        mode = self.action_var.get()
        with self.pool_lock: pool_empty = not self.user_pool
        if pool_empty and "Strict Check-in" in mode:
            self.log("FATAL: attendee pool is empty. Reload the pool, register real attendees, or switch to Registrations / Mixed mode first.")
            return

        event_days_raw = [d.strip() for d in self.event_days_var.get().split(",") if d.strip()]
        event_year = self._safe_int(self.event_year_var, DEFAULT_EVENT_YEAR)
        event_dates = parse_event_days(event_days_raw, event_year)
        if not event_dates:
            self.log("WARNING: no valid event days configured — falling back to defaults.")
        ...
        self.log(f"Test initialized — spawning {total_threads} virtual devices across {len(targets)} target(s). Pool: {len(snapshot)} real attendees.")
        ...
        for env_name, base_url in targets:
            for _ in range(dev_count):
                ...
                threading.Thread(
                    ...
                    args=(env_name, base_url, thread_id_counter, run_config, delay),
                    daemon=True,
                ).start()
                thread_id_counter += 1
    ...
    def stop_test(self):
        ...
        self.log("Halt requested — in-flight requests will finish, then all devices stop.")
    ...
    def reset_stats(self):
        ...
        for lbl in self.err_labels.values(): lbl.config(text="0")
    ...
    def api_worker(self, env_name, base_url, thread_id, run_config: RunConfig, start_delay):
        ...
        try:
            while self.is_running:
                ...
                if "Mixed Load" in run_config.mode: is_checkin = random.random() < 0.50
                elif "Strict Check-in" in run_config.mode: is_checkin = True
                ...
                if is_checkin:
                    try: user = self.user_queue.get_nowait()
                        ...
                        if pool_snapshot: user = random.choice(pool_snapshot)
                            ...
                            self.log("FATAL: attendee pool exhausted mid-run. Halting.")
                            self.is_running = False
                            break
                        else: is_checkin = False
                ...
                try:
                    if is_checkin:
                        ...
                        resp = session.post(url, json=payload, timeout=run_config.timeout, verify=False)
                    ...
                    self.stats_queue.put(("TIMEOUT", 0, env_name, "error", "-"))
                except requests.exceptions.ConnectionError:
                    self.stats_queue.put(("CONN_REFUSED", 0, env_name, "error", "-"))
                except requests.exceptions.RequestException:
                    self.stats_queue.put(("NETWORK_ERR", 0, env_name, "error", "-"))
    ...
    def update_gui_loop(self):
        ...
        if self.is_running:
            ...
            progress = min(100, (elapsed / self.test_duration) * 100) if self.test_duration else 0
    ...
    def _update_analytics_column(self, widgets, data):
        ...
        apdex_style = "success" if apdex >= 0.85 else ("warning" if apdex >= 0.6 else "danger")
        widgets["Apdex Score (<500ms):"].config(text=f"{apdex:.2f}", bootstyle=apdex_style)
        widgets["P50 (Median) ms:"].config(text=f"{calculate_percentile(sorted_data, 50):.0f}")
        widgets["P90 ms:"].config(text=f"{calculate_percentile(sorted_data, 90):.0f}")
        p95 = calculate_percentile(sorted_data, 95)
        widgets["P95 ms (Warning):"].config(text=f"{p95:.0f}", bootstyle="warning" if p95 > 1000 else "default")
        p99 = calculate_percentile(sorted_data, 99)
        widgets["P99 ms (Critical):"].config(text=f"{p99:.0f}", bootstyle="danger" if p99 > 1500 else "default")
    ...
    def _build_report_text(self):
        ...
        for label, data in (("CHECK-IN", rts_checkin_snap), ("REGISTRATION", rts_register_snap)):
            ...
            lines.append(f"  P50: {calculate_percentile(sd, 50):.0f}ms   P90: {calculate_percentile(sd, 90):.0f}ms   P95: {calculate_percentile(sd, 95):.0f}ms   P99: {calculate_percentile(sd, 99):.0f}ms")
            lines.append(f"  Min/Max: {sd[0]:.0f}ms / {sd[-1]:.0f}ms\n")

        with self.pool_lock:
            pool_n = len(self.user_pool)
            counts = dict(self.pool_counts)
        detail_str = ", ".join(f"{v} from {k}" for k, v in counts.items()) if counts else "n/a"
        lines.append(f"Attendee pool used: {pool_n} real identities ({detail_str})")
        lines.append("=" * 72)
        return "\n".join(lines)
