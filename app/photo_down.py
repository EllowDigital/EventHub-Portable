...
logging.basicConfig(
    handlers=[RotatingFileHandler(os.path.join(
        LOG_DIR, 'photo_downloader.log'), maxBytes=5_000_000, backupCount=2)],
    level=logging.INFO,
...
class PhotoDownloadManager:
    ...
    def mirror_photos_to_sqlite(self):
        ...
        try:
            ...
            data_dicts = [
                {c.name: getattr(p, c.name) for c in p.__table__.columns} for p in mysql_photos]
            sqlite_session.query(DownloadedPhoto).delete()
            if data_dicts:
                sqlite_session.bulk_insert_mappings(
                    DownloadedPhoto, data_dicts)
            sqlite_session.commit()
            logging.info(
                f"Photo mirror complete: {len(data_dicts)} records backed up to SQLite.")
        except Exception as e:
    ...
    def get_stats(self):
        ...
        try:
            ...
            downloaded_map = {
                record.attendee_id: record for record in downloaded_records}
    ...
    def process_photos(self, update_progress_callback, finished_callback):
        ...
        try:
            ...
            if total_pending == 0:
                logging.info(
                    "Everything is up to date! No downloads or syncs needed.")
                finished_callback(True)
                return

            logging.info(
                f"Processing {total_pending} items ({len(stats['pending_db_sync'])} local-sync, {len(stats['pending_download'])} cloud-download)...")
            existing_map = {p.attendee_id: p for p in session.query(
                DownloadedPhoto).all()}
            success_count = 0
            ...
            for index, att in enumerate(to_process):
                ...
                relative_path = os.path.relpath(
                    absolute_path, BASE_DIR).replace('\\', '/')
                ...
                try:
                    ...
                    file_size_kb = round(
                        os.path.getsize(absolute_path) / 1024, 2)
                    ...
                    if existing_record:
                        ...
                        existing_record.downloaded_at = datetime.now(
                    ...
                    success_count += 1
                    logging.info(
                        f"[{att.attendee_id}] {action_msg} ({file_size_kb} KB)")
                except requests.exceptions.RequestException as req_err:
                    logging.error(
                        f"[{att.attendee_id}] Network Error downloading: {req_err}")
                except Exception as e:
            ...
            logging.info(
                f"Finished. Successfully processed {success_count}/{total_pending} photos.")
            if success_count > 0:
...
class PhotoDownloaderGUI(QMainWindow):
    ...
    def build_ui(self):
        ...
        btn_refresh.setStyleSheet(
            f"border: 1px solid {COLORS['INFO']}; color: {COLORS['INFO']}; background: transparent; padding: 8px 16px;")
        ...
        self._create_stat_card(
            cards_frame, "📸", "TOTAL PROFILES", "0", COLORS["INFO"], "total")
        self._create_stat_card(
            cards_frame, "💾", "FULLY SYNCED", "0", COLORS["SUCCESS"], "synced")
        self._create_stat_card(
            cards_frame, "🔍", "LOCAL FILES UNLINKED", "0", COLORS["WARNING"], "pending_db_sync")
        self._create_stat_card(
            cards_frame, "☁️", "CLOUD DOWNLOADS", "0", COLORS["DANGER"], "pending_download")
        ...
        self.btn_start.setStyleSheet(
            f"background-color: {COLORS['SUCCESS']}; color: white; padding: 10px;")
        self.btn_start.clicked.connect(self.start_download)

        self.btn_stop = QPushButton("⏹ Stop")
        self.btn_stop.setFixedWidth(100)
        self.btn_stop.setStyleSheet(
            f"background-color: transparent; color: {COLORS['DANGER']}; border: 1px solid {COLORS['DANGER']}; padding: 10px;")
        ...
        lbl_log.setStyleSheet(
            f"color: {COLORS['SECONDARY']}; font-weight: bold; font-size: 11px;")
        ...
        self.log(
            "Photo Engine ready. Run a stat refresh to detect unlinked local files.", "SUCCESS")
    ...
    def _create_stat_card(self, parent_layout, icon, title, initial_value, color, var_name):
        ...
        stripe.setStyleSheet(
            f"background-color: {color}; border-top-left-radius: 4px; border-bottom-left-radius: 4px;")
    ...
    def animate_stat_flash(self, var_name):
        ...
        QTimer.singleShot(150, lambda: lbl.setStyleSheet(
            f"color: {original_color}; border: none;") if lbl else None)
    ...
    def _apply_stats(self, stats):
        for key in ["total", "synced", "pending_db_sync", "pending_download"]:
            new_val = str(stats[key] if key == "total" or key ==
                          "synced" else len(stats[key]))
            lbl = self.stat_vars[key]["label"]
            if lbl.text() != new_val:
                lbl.setText(new_val)
                self.animate_stat_flash(key)

        pending_total = len(stats["pending_db_sync"]) + \
            len(stats["pending_download"])
        self.progress.setValue(0)
    ...
    def start_download(self):
        ...
        self.btn_start.setStyleSheet(
            f"background-color: transparent; color: {COLORS['SUCCESS']}; border: 1px solid {COLORS['SUCCESS']}; padding: 10px;")
        self.btn_stop.setEnabled(True)
        self.btn_stop.setStyleSheet(
            f"background-color: {COLORS['DANGER']}; color: white; padding: 10px;")
    ...
    if os.name == 'nt':
        try:
            my_app_id = os.environ.get(
                "EVENTHUB_TOOL_ID", "EventHub.Tool.photos")
