...
import csv
import math
import queue  # <-- Restored missing import
import random
import threading
import time
import uuid
...
def build_verdict(total, success_200, server_errors, p95, apdex):
    if total == 0:
        return "⚪ IDLE", "System armed and awaiting load injection commands."
...
def load_full_attendee_pool(session_factories, cap_per_table, log_signal):
    ...
    for backend_name in ("mysql", "sqlite"):
        ...
        log_signal.emit(
            f"[{backend_name.upper()}] Loaded {added} real attendees ({counts['attendees']} base, {counts['kiosk']} kiosk).")
...
class AppSignals(QObject):
    log_msg = Signal(str)
    # metrics, chk_rts, reg_rts, tree_items, live_rps, peak_rps
    metrics_update = Signal(dict, list, list, list, int, int)
...
class SpeedometerGauge(QWidget):
    ...
    def paintEvent(self, event):
        ...
        margin_y = 5.0
        arc_rect = QRectF(margin_x + 6, margin_y + 6,
                          diameter - 12, diameter - 12)
        arc_width = max(5.0, diameter * 0.08)

        ratio = min(
            max(self.current_val / self.max_val if self.max_val > 0 else 0.0, 0.0), 1.0)

        if self.good_is_high:
            dynamic_color = QColor(
                "#4EC9B0") if ratio > 0.5 else QColor("#D7BA7D")
        else:
            dynamic_color = QColor("#F44747") if ratio > 0.85 else (
                QColor("#D7BA7D") if ratio > 0.6 else QColor("#4EC9B0"))
        ...
        val_rect = QRectF(margin_x, margin_y + (diameter *
                          0.2), diameter, diameter * 0.45)
...
class EnterpriseStressTestApp(QMainWindow):
    ...
    def _emit_metrics(self, now):
        with self.data_lock:
            ...
            if live_rps > self.peak_rps:
                self.peak_rps = live_rps
            ...
            self.signals.metrics_update.emit(
                metrics_snap, chk_snap, reg_snap, tree_snap, live_rps, self.peak_rps)
    ...
    def reload_attendee_pool(self, initial=False):
        if self.is_running:
            return self.signals.log_msg.emit("Stop test before reloading pool.")

        self.signals.btn_state.emit("btn_reload", "⏳ LOADING...", False)
        if not initial:
            self.signals.btn_state.emit("btn_start", "▶ INJECT LOAD", False)

        cap = self.pool_cap_var.value()
        self.signals.log_msg.emit(
            "Connecting to database to load the real attendee pool...")
    ...
    def _load_pool_worker(self, cap):
        try:
            ...
            pool, counts = load_full_attendee_pool(
                sessions, cap, log_signal=self.signals.log_msg)
            with self.pool_lock:
                self.user_pool = pool
                self.pool_counts = counts
            if pool:
                self.signals.log_msg.emit(
                    f"SUCCESS: {len(pool)} real attendees loaded and available for check-in testing.")
            else:
                self.signals.log_msg.emit(
                    "WARNING: No attendees found. Run a Registration load test to create synthetic attendees first.")
        except Exception as e:
            self.signals.log_msg.emit(f"ERROR loading attendee pool: {e}")
        finally:
            self.signals.pool_loaded.emit(
                len(self.user_pool), dict(self.pool_counts))
            ...
            QTimer.singleShot(2000, lambda: self.signals.btn_state.emit(
                "btn_reload", "🔄 Reload Attendee Pool", True))

    def on_reset_synthetic_history(self):
        if self.is_running:
            return self.signals.log_msg.emit("Stop test before modifying database.")
        res = QMessageBox.warning(
            self, "Reset Test History", f"Clear checkin_history ONLY for '{SYNTHETIC_NAME_PREFIX} *' attendees?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def _reset_synth_worker(self):
        try:
            sessions = self._get_sessions()
            touched = sum(1 for backend in ("mysql", "sqlite") if sessions.get(
                backend) and not reset_synthetic_checkin_history(sessions.get(backend)))
            self.signals.log_msg.emit(
                f"Test check-in history reset on active backend(s).")
        except Exception as e:
            self.signals.log_msg.emit(f"ERROR resetting history: {e}")
        finally:
            self.signals.btn_state.emit("btn_reset_synth", "✅ CLEARED", True)
            QTimer.singleShot(2000, lambda: self.signals.btn_state.emit(
                "btn_reset_synth", "🧹 Reset History", True))

    def on_purge_synthetic(self):
        if self.is_running:
            return self.signals.log_msg.emit("Stop test before modifying database.")
        res = QMessageBox.warning(self, "Purge Synthetic Data",
                                  f"PERMANENTLY DELETE every '{SYNTHETIC_NAME_PREFIX} *' attendee? Cannot be undone.", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if res != QMessageBox.StandardButton.Yes:
            return
        ...
        try:
            ...
            for backend in ("mysql", "sqlite"):
                ...
                if factory:
                    ...
                    total_k += n2
            self.signals.log_msg.emit(
                f"Purged {total_a} main attendees + {total_k} kiosk records.")
            self.reload_attendee_pool()
        except Exception as e:
            self.signals.log_msg.emit(f"ERROR purging data: {e}")
        finally:
            self.signals.btn_state.emit("btn_purge_synth", "✅ PURGED", True)
            QTimer.singleShot(2000, lambda: self.signals.btn_state.emit(
                "btn_purge_synth", "🗑️ Purge Data", True))

    def on_reset_all_history(self):
        if self.is_running:
            return self.signals.log_msg.emit("Stop test before modifying database.")
        res = QMessageBox.warning(self, "Wipe ALL Check-ins", "DANGER: Erase check-in history for EVERY attendee globally?",
    ...
    def _reset_all_worker(self):
        try:
            ...
            self.signals.log_msg.emit(f"ERROR wiping history: {e}")
        finally:
            self.signals.btn_state.emit("btn_reset_all", "✅ WIPED", True)
            QTimer.singleShot(2000, lambda: self.signals.btn_state.emit(
                "btn_reset_all", "☢️ Wipe ALL Check-ins", True))

    def on_clear_kiosk(self):
        if self.is_running:
            return self.signals.log_msg.emit("Stop test before modifying database.")
        res = QMessageBox.warning(self, "Clear Kiosk", "DANGER: Delete EVERY registration record from the Offline Kiosk table?",
                                  QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if res != QMessageBox.StandardButton.Yes:
            return
    ...
    def _clear_kiosk_worker(self):
        try:
            sessions = self._get_sessions()
            total_k = sum(clear_all_kiosk_registrations(sessions.get(backend))
                          for backend in ("mysql", "sqlite") if sessions.get(backend))
            self.signals.log_msg.emit(
                f"Cleared {total_k} total kiosk registrations.")
            self.reload_attendee_pool()
        except Exception as e:
            self.signals.log_msg.emit(f"ERROR clearing kiosk: {e}")
        finally:
            self.signals.btn_state.emit("btn_clear_kiosk", "✅ CLEARED", True)
            QTimer.singleShot(2000, lambda: self.signals.btn_state.emit(
                "btn_clear_kiosk", "☢️ Clear ALL Kiosk Reg", True))
    ...
    def _build_control_panel(self):
        ...
        self.action_var.addItems(["Strict Registrations (Kiosk Simulation)",
                                 "Strict Check-ins (Scanner Simulation)", "Mixed Load (50% Check-in / 50% Reg)"])
        lay_profile.addRow("Action Distribution:", self.action_var)

        self.sync_var = QComboBox()
        self.sync_var.addItems(["Synchronized Millisecond Stampede",
                               "Human Pacing (1-3s delays)", "Gradual Ramp-Up (Stress Growth)"])
        ...
        self.threads_var = QSpinBox()
        self.threads_var.setRange(1, 1000)
        self.threads_var.setValue(18)
        self.duration_var = QSpinBox()
        self.duration_var.setRange(1, 3600)
        self.duration_var.setValue(60)
        self.rampup_var = QSpinBox()
        self.rampup_var.setRange(0, 3600)
        self.rampup_var.setValue(5)
        self.timeout_var = QDoubleSpinBox()
        self.timeout_var.setRange(0.1, 60.0)
        self.timeout_var.setValue(8.0)
        ...
        h_cap = QHBoxLayout()
        lbl_cap = QLabel("Max rows to load:")
        lbl_cap.setStyleSheet("font-weight: bold;")
        self.pool_cap_var = QSpinBox()
        self.pool_cap_var.setRange(1, 1000000)
        self.pool_cap_var.setValue(DEFAULT_POOL_CAP)
        h_cap.addWidget(lbl_cap)
        h_cap.addWidget(self.pool_cap_var)
        lay_db.addLayout(h_cap)

        self.btn_reload = QPushButton("🔄 Reload Attendee Pool")
        ...
        self.btn_reload.clicked.connect(
            lambda: self.reload_attendee_pool(initial=False))
        lay_db.addWidget(self.btn_reload)

        lbl_synth = QLabel("Synthetic Test Data:")
        lbl_synth.setStyleSheet("font-weight: bold; margin-top: 10px;")
        lay_db.addWidget(lbl_synth)
        h_synth = QHBoxLayout()
        self.btn_reset_synth = QPushButton("🧹 Reset History")
        self.btn_reset_synth.setObjectName("btn_reset_synth")
        self.btn_reset_synth.clicked.connect(self.on_reset_synthetic_history)
        self.btn_purge_synth = QPushButton("🗑️ Purge Data")
        self.btn_purge_synth.setObjectName("btn_purge_synth")
        self.btn_purge_synth.setStyleSheet(
            "color: #F44747; border-color: #F44747;")
        self.btn_purge_synth.clicked.connect(self.on_purge_synthetic)
        h_synth.addWidget(self.btn_reset_synth)
        h_synth.addWidget(self.btn_purge_synth)
        lay_db.addLayout(h_synth)

        lbl_global = QLabel("Global Live Data (DANGER):")
        lbl_global.setStyleSheet(
            "font-weight: bold; color: #F44747; margin-top: 10px;")
        lay_db.addWidget(lbl_global)
        h_global = QHBoxLayout()
        self.btn_reset_all = QPushButton("☢️ Wipe ALL Check-ins")
        self.btn_reset_all.setObjectName("btn_reset_all")
        self.btn_reset_all.setStyleSheet(
            "background-color: #8C2323; color: white;")
        self.btn_reset_all.clicked.connect(self.on_reset_all_history)
        self.btn_clear_kiosk = QPushButton("☢️ Clear ALL Kiosk Reg")
        self.btn_clear_kiosk.setObjectName("btn_clear_kiosk")
        self.btn_clear_kiosk.setStyleSheet(
            "background-color: #8C2323; color: white;")
        self.btn_clear_kiosk.clicked.connect(self.on_clear_kiosk)
        h_global.addWidget(self.btn_reset_all)
        h_global.addWidget(self.btn_clear_kiosk)
        ...
        # Execution Buttons
        h_exec = QHBoxLayout()
        self.btn_start = QPushButton("▶ INJECT LOAD")
    ...
    def _build_dashboard_tab(self):
        ...
        self.meter_chk = SpeedometerGauge(
            "Check-ins / sec", "RPS", 50, good_is_high=True)
        self.meter_reg = SpeedometerGauge(
            "Registrations / sec", "RPS", 50, good_is_high=True)
        h_gauges.addWidget(self.meter_chk)
        h_gauges.addWidget(self.meter_reg)
        layout.addLayout(h_gauges)

        # Metric Cards
        h_cards = QHBoxLayout()
        card_defs = [("Total Reqs", "#569CD6"), ("Success %", "#4EC9B0"),
                     ("Avg RT (ms)", "#569CD6"), ("Live RPS", "#D7BA7D"), ("Peak RPS", "#F44747")]
        for name, color in card_defs:
            f = QFrame()
            f.setStyleSheet(
                "QFrame { background:#1E1E22; border:1px solid #28282B; border-radius:4px; }")
            l = QVBoxLayout(f)
            l.setContentsMargins(8, 8, 8, 8)
            l.setSpacing(2)
            lbl_title = QLabel(name)
            lbl_title.setStyleSheet(
                "color: #858585; font-size: 11px; font-weight: bold; border:none;")
            l.addWidget(lbl_title, alignment=Qt.AlignCenter)
            val = QLabel("0")
            val.setStyleSheet(
                f"color:{color}; font-size:24px; font-weight:bold; border:none;")
        ...
        if MATPLOTLIB_AVAILABLE:
            fig, (self.ax_tps, self.ax_rt) = plt.subplots(
                2, 1, figsize=(8, 4), facecolor="#161618")
            ...
            lbl_no_graph = QLabel(
                "(Install 'matplotlib' via pip to view Live Traffic Graph)")
            lbl_no_graph.setAlignment(Qt.AlignCenter)
            lbl_no_graph.setStyleSheet("color: #888888; font-style: italic;")
    ...
    def _build_analytics_tab(self):
        ...
        lbl_desc = QLabel(
            "Analytics panel for Check-in (reads/updates) and Registration (writes) tracked separately.")
        lbl_desc.setStyleSheet(
            "color: #858585; font-style: italic; margin-bottom: 10px;")
        ...
        err_defs = [
            ("dup_400", "HTTP 400 — Duplicate / Client Rejection:"), ("denied_403",
                                                                      "HTTP 403 — Access Denied (wrong date):"),
            ("notfound_404", "HTTP 404 — Attendee Not Found:"), ("server_5xx",
                                                                 "HTTP 500+ — Server Fatality:"),
            ("conn_refused", "Connection Refused / Unreachable:"), ("timeouts",
                                                                    "Timed Out (no limit):"),
        ]
        for i, (key, label_text) in enumerate(err_defs):
            ...
            lbl_v = QLabel("0")
            lbl_v.setStyleSheet("font-weight: bold; color: #D7BA7D;")
    ...
    def _build_metrics_column(self, layout):
        ...
        for label_text in rows:
            ...
            val = QLabel("N/A")
            val.setStyleSheet("font-weight: bold; color: #555555;")
            h.addWidget(lbl)
            h.addStretch()
            h.addWidget(val)
            layout.addLayout(h)
    ...
    def _build_grid_tab(self):
        ...
        self.tree.setHorizontalHeaderLabels(
            ["Time", "Target", "Action", "Status", "RT (ms)", "Identifier Used"])
    ...
    def start_test(self):
        ...
        if not targets:
            return self.signals.log_msg.emit("ERROR: no routing targets provided. Fill in at least one target URL.")
        bad = [url for _, url in targets if not (
            url.startswith("http://") or url.startswith("https://"))]
        if bad:
            self.signals.log_msg.emit(
                f"WARNING: target(s) missing http(s):// scheme: {', '.join(bad)}")
        ...
        event_days_raw = [
            d.strip() for d in self.event_days_var.text().split(",") if d.strip()]
        try:
            event_year = int(self.event_year_var.text())
        except ValueError:
            event_year = DEFAULT_EVENT_YEAR

        event_dates = parse_event_days(event_days_raw, event_year)
        if not event_dates:
            event_dates = parse_event_days(
                DEFAULT_EVENT_DAYS.split(","), DEFAULT_EVENT_YEAR)

        self.signals.reset_ui.emit()

        # --- UI STATE UPDATE (START) ---
        self.signals.btn_state.emit("btn_start", "✅ RUNNING...", False)
        self.signals.btn_state.emit(
            "btn_reload", "🔄 Reload Attendee Pool", False)
        ...
        run_config = RunConfig(mode=mode, sync_mode=sync_mode, timeout=timeout_limit,
                               event_day_labels=tuple(event_days_raw), event_dates=tuple(event_dates))

        self.signals.log_msg.emit(
            f"Test initialized — {total_threads} virtual devices on {len(targets)} target(s).")

        thread_id_counter = 0
        for env_name, base_url in targets:
            for _ in range(dev_count):
                delay = (thread_id_counter / (total_threads - 1)) * \
                    ramp_up_sec if (
                        "Ramp-Up" in sync_mode and total_threads > 1) else 0.0
                threading.Thread(target=self.api_worker, args=(
                    env_name, base_url, thread_id_counter, run_config, delay), daemon=True).start()
                thread_id_counter += 1
    ...
    def stop_test(self):
        ...
        self.signals.log_msg.emit(
            "Halt requested — waiting for in-flight requests to finish...")
    ...
    def api_worker(self, env_name, base_url, thread_id, run_config: RunConfig, start_delay):
        ...
        adapter = HTTPAdapter(pool_connections=500,
                              pool_maxsize=500, max_retries=retries)
        ...
        try:
            while self.is_running:
                ...
                is_checkin = ("Strict Check-in" in run_config.mode) or (
                    "Mixed Load" in run_config.mode and random.random() < 0.50)
                user = None
                ...
                try:
                    if is_checkin:
                        ...
                        resp = session.post(
                            url_checkin, json=payload, timeout=run_config.timeout, verify=False)
                        identifier = payload["attendee_id"]
                    else:
                        payload = generate_registration_payload(
                            device_name, device_id, run_config.event_day_labels)
                        resp = session.post(
                            url_register, json=payload, timeout=run_config.timeout, verify=False)
                ...
                if self.is_running and self.test_duration and (time.time() - self.start_time) >= self.test_duration:
                    self.signals.log_msg.emit(
                        "Time limit reached. Halting traffic...")
                    self.stop_test()
                    break
        finally:
            session.close()
    ...
    def _ui_update_metrics(self, metrics_snap, chk_snap, reg_snap, tree_snap, live_rps, peak_rps):
        ...
        self.stat_widgets["Success %"].setStyleSheet(
            f"font-size:24px; font-weight:bold; border:none; color: {'#4EC9B0' if success_rate >= 98 else ('#D7BA7D' if success_rate >= 90 else '#F44747')};")
        ...
        for key, lbl in self.err_labels.items():
            lbl.setText(str(metrics_snap.get(key, 0)))
        ...
        if "CRITICAL" in level:
            self.lbl_verdict.setStyleSheet(
                "color: #F44747; font-size: 16px; font-weight: bold;")
        elif "DEGRADED" in level:
            self.lbl_verdict.setStyleSheet(
                "color: #D7BA7D; font-size: 16px; font-weight: bold;")
        elif "HEALTHY" in level:
            self.lbl_verdict.setStyleSheet(
        ...
        if tree_snap:
            ...
            for item in tree_snap:
                ...
                code = item[3]
                color = QColor("#4EC9B0") if code == 200 else (QColor(
                    "#D7BA7D") if isinstance(code, int) and code < 500 else QColor("#F44747"))
                for col, val in enumerate(item):
                    tw_item = QTableWidgetItem(str(val))
                    tw_item.setForeground(color)
                    self.tree.setItem(0, col, tw_item)
            while self.tree.rowCount() > 50:
                self.tree.removeRow(50)
        ...
        if MATPLOTLIB_AVAILABLE and hasattr(self, 'line_tps'):
            ...
            if self.tabs.currentIndex() == 0:
                self.line_tps.set_data(
                    range(len(self.throughput_history)), list(self.throughput_history))
                self.ax_tps.relim()
                self.ax_tps.autoscale_view()
                self.line_rt.set_data(
                    range(len(self.plot_rts_history)), list(self.plot_rts_history))
                self.ax_rt.relim()
    ...
    def reset_stats(self):
        ...
        self.meter_chk.set_target(0)
        self.meter_chk.current_val = 0
        self.meter_reg.set_target(0)
        self.meter_reg.current_val = 0
        ...
        self.lbl_verdict.setStyleSheet(
            "color: #D4D4D4; font-size: 16px; font-weight: bold;")

    def _update_analytics_column(self, widgets, data):
        n = len(data)
        widgets["Total Processed:"].setText(str(n))
        widgets["Total Processed:"].setStyleSheet(
            "font-weight: bold; color: #569CD6;")
        ...
        apdex_color = "#4EC9B0" if apdex >= 0.85 else (
            "#D7BA7D" if apdex >= 0.6 else "#F44747")
        widgets["Apdex Score (<500ms):"].setText(f"{apdex:.2f}")
        widgets["Apdex Score (<500ms):"].setStyleSheet(
            f"font-weight: bold; color: {apdex_color};")

        widgets["P50 (Median) ms:"].setText(
            f"{calculate_percentile(sorted_data, 50):.0f}")
        widgets["P50 (Median) ms:"].setStyleSheet(
            "font-weight: bold; color: #569CD6;")

        widgets["P90 ms:"].setText(
            f"{calculate_percentile(sorted_data, 90):.0f}")
        ...
        widgets["P95 ms (Warning):"].setStyleSheet(
            f"font-weight: bold; color: {'#D7BA7D' if p95 > 1000 else '#569CD6'};")

        p99 = calculate_percentile(sorted_data, 99)
        widgets["P99 ms (Critical):"].setText(f"{p99:.0f}")
        widgets["P99 ms (Critical):"].setStyleSheet(
            f"font-weight: bold; color: {'#F44747' if p99 > 1500 else '#569CD6'};")

        widgets["Min / Max ms:"].setText(
            f"{sorted_data[0]:.0f} / {sorted_data[-1]:.0f}")
        widgets["Min / Max ms:"].setStyleSheet(
            "font-weight: bold; color: #569CD6;")
    ...
    def export_summary_report(self):
        with self.data_lock:
            ...
            if total == 0:
                return self.signals.log_msg.emit("Nothing to export yet — run a test first.")

        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Text Report", "", "Text Report (*.txt)")
        if not file_path:
            return
        ...
        try:
            ...
            QTimer.singleShot(2500, lambda: self.signals.btn_state.emit(
                "btn_rep", "📄 Export Summary Report", True))
    ...
    def _build_report_text(self):
        ...
        lines.append(
            f"Attendee pool used: {pool_n} real identities ({', '.join(f'{v} from {k}' for k, v in counts.items()) if counts else 'n/a'})")
        lines.append("=" * 72)
        return "\n".join(lines)
    ...
    if os.name == 'nt':
        try:
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
                os.environ.get("EVENTHUB_TOOL_ID", "EventHub.Tool.stress"))
        except Exception:
            pass

    # Correct Initialization Sequence to satisfy PySide6 HighDPI Rules
