...
def global_exception_handler(exc_type, exc_value, exc_traceback):
    logging.error("Uncaught GUI Exception intercepted. App remains running.",
...
class NotificationEngine(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            speak_text = "Access Denied." if status not in [
                "SUCCESS", "DUPLICATE", "ALERT"] else ""
            if speak_text:
                if platform.system() == "Windows":
...
class SettingsDialog(QDialog):
    ...
    def build_ui(self):
        ...
        fields = [
            ("Hub Server URL (HTTP/HTTPS)", "hub_url",
             self.config_manager.config["hub_url"]),
            ("Device Identifier Name", "device_name",
             self.config_manager.config["device_name"])
        ]
        ...
        self.ent_photo = QLineEdit(
            self.config_manager.config["photo_directory"])
        ...
        btn_save.setStyleSheet(
            f"background-color: {THEMES['darkly']['SUCCESS']}; color: white; font-weight: bold;")
    ...
    def browse_dir(self):
        start_dir = os.path.normpath(
            os.path.join(BASE_DIR, self.ent_photo.text()))
        d = QFileDialog.getExistingDirectory(
            self, "Select Directory", start_dir)
        if d:
            self.ent_photo.setText(os.path.relpath(
                d, start=BASE_DIR).replace('\\', '/'))

    def save(self):
        self.config_manager.config["hub_url"] = self.entries["hub_url"].text(
        ).strip().rstrip('/')
        self.config_manager.config["device_name"] = self.entries["device_name"].text(
        ).strip()
        self.config_manager.config["photo_directory"] = self.ent_photo.text(
        ).strip()
...
class GateDisplay(QMainWindow):
    def __init__(self):
        ...
        self.stats = {"Success": 0, "Duplicate": 0,
                      "Wrong Day": 0, "Errors": 0}
        self.recent_scans = []
    ...
    def build_ui(self):
        ...
        self.lbl_subtitle = QLabel(
            f"{self.config_manager.config['device_name']} • TDE UP 2026")
        ...
        btn_fs.clicked.connect(lambda: self.showNormal(
        ) if self.isFullScreen() else self.showFullScreen())
        ...
        self.net_dot.setStyleSheet(
            f"color: {THEMES['darkly']['WARNING']}; font-size: 16px;")
        ...
        self.test_banner.setStyleSheet(
            f"background-color: {THEMES['darkly']['DANGER']}; color: white; padding: 10px;")
        ...
        self.status_banner.setStyleSheet(
            f"background-color: {THEMES['darkly']['SECONDARY']}; color: white;")
        ...
        self.lbl_pass_badge.setStyleSheet(
            f"background-color: {THEMES['darkly']['SECONDARY']}; color: white; padding: 10px 20px; border-radius: 8px;")
        ...
        btn_proc.setStyleSheet(
            f"background-color: {THEMES['darkly']['SUCCESS']}; color: white; padding: 12px; font-size: 11pt;")
        ...
        # CRITICAL: Prevents stretching/overlapping
    ...
    def toggle_sound(self):
        ...
        if self.notifier.sound_enabled:
            ...
            self.btn_sound.setStyleSheet(
                f"border: 1px solid {THEMES['darkly']['INFO']}; color: {THEMES['darkly']['INFO']}; background: transparent;")
        else:
            self.btn_sound.setText("🔇 Muted")
            self.btn_sound.setStyleSheet(
                f"border: 1px solid {THEMES['darkly']['SECONDARY']}; color: {THEMES['darkly']['SECONDARY']}; background: transparent;")
    ...
    def async_load_photo(self, attendee_id):
        ...
        def _load():
            rel_dir = self.config_manager.config.get(
                "photo_directory", DEFAULT_PHOTO_DIR)
    ...
    def process_queues(self):
        while not self.gui_queue.empty():
            try:
                ...
                logging.error(f"GUI queue task failed: {e}")

        scans_to_process = []
        while not self.scan_queue.empty() and len(scans_to_process) < 5:
            try:
                scans_to_process.append(self.scan_queue.get_nowait())
            except queue.Empty:
                break
    ...
    def trigger_banner_animation(self, c_style):
        ...
        self.status_banner.setStyleSheet(
            f"background-color: {flash_color}; color: {t['BG']}; border-radius: 8px;")
        QTimer.singleShot(150, lambda: self.status_banner.setStyleSheet(
            f"background-color: {original_color}; color: white; border-radius: 8px;"))
    ...
    def update_ui_with_event(self, event_data):
        ...
        self.bottom_banner.setStyleSheet(
            f"background-color: {banner_color}; color: white; border-radius: 8px;")
        ...
        if attendee:
            ...
            badge_style = CATEGORY_STYLES.get(
                category_raw, CATEGORY_STYLES["default"])
            badge_color = t.get(badge_style, t['SECONDARY'])

            self.lbl_pass_badge.setText(
                category_raw.upper() if category_raw else "UNKNOWN")
            self.lbl_pass_badge.setStyleSheet(
                f"background-color: {badge_color}; color: white; padding: 10px 20px; border-radius: 8px;")
            self.lbl_name.setText(attendee.get("full_name", "").upper())

            comp_color = t['INFO'] if c_style == "SUCCESS" else banner_color
            self.lbl_company.setText(attendee.get(
                "business_name") or "General Admission")
            self.lbl_company.setStyleSheet(
                f"color: {comp_color}; font-weight: bold;")

            self.lbl_attendee_id.setText(attendee.get("attendee_id", ""))

            mobile = str(attendee.get("mobile", ""))
            masked_mobile = f"••••••{mobile[-4:]}" if len(
                mobile) >= 4 else mobile
            self.fields["mobile"].setText(masked_mobile)
            self.fields["location"].setText(
                f"{attendee.get('city', '')}, {attendee.get('state', '')}".strip(', '))
            ...
            self.add_recent_scan(attendee.get("full_name"), attendee.get(
                "attendee_id"), c_style, time_str)
        else:
            ...
            self.lbl_pass_badge.setStyleSheet(
                f"background-color: {t['SECONDARY']}; color: white; padding: 10px 20px; border-radius: 8px;")
            for lbl in self.fields.values():
                lbl.setText("---")
            self.set_placeholder_photo()

        if status_type in ["SUCCESS", "DUPLICATE", "ERROR"]:
            key = "Success" if status_type == "SUCCESS" else (
                "Duplicate" if status_type == "DUPLICATE" else "Errors")
            self.stats[key] += 1
    ...
    def add_recent_scan(self, name, att_id, style, time_str):
        ...
        card.setStyleSheet(
            f"background-color: {bg_color}; color: white; border-radius: 6px;")
    ...
    def show_hub_message(self, msg):
        if getattr(self, '_showing_msg', False):
            return
        ...
        frame = QFrame()
        frame.setStyleSheet(
            f"border: 3px solid {THEMES['darkly']['WARNING']}; background-color: {THEMES['darkly']['CARD_BG']}; border-radius: 6px;")
        ...
        btn.setStyleSheet(
            "background-color: #333; padding: 10px; font-weight: bold;")
    ...
    def get_system_telemetry(self):
        ...
        if HAS_PSUTIL:
            try:
                ...
                if batt is not None:
                    battery_str = f"{int(batt.percent)}%" + \
                        (" AC" if batt.power_plugged else "")
                else:
            ...
            if temp_str == "N/A" and platform.system() == "Windows":
                try:
                    ...
                    output = subprocess.check_output(
                        cmd, shell=True, stderr=subprocess.DEVNULL, timeout=2).decode().strip()
    ...
    def start_threads(self):
        ...
        self.stream_session.headers.update(
            {"User-Agent": "EventHub-GateDisplay-Stream/2.6", "Connection": "keep-alive"})
        self.api_session = requests.Session()
        self.api_session.headers.update(
            {"User-Agent": "EventHub-GateDisplay-API/2.6", "Connection": "keep-alive"})

        self.is_polling = True
        threading.Thread(target=self.telemetry_loop, daemon=True).start()
        threading.Thread(target=self.listen_to_server_stream,
                         daemon=True).start()
    ...
    def poll_server_status(self):
        while self.is_polling:
            try:
                hub_url = self.config_manager.config.get(
                    'hub_url', '').rstrip('/')
                payload = {
                ...
                resp = self.api_session.post(
                    f"{hub_url}/api/status", json=payload, timeout=3, verify=False)
                ...
                if canonical and canonical != self.config_manager.config.get("device_name") and canonical != "Unknown Device":
                    ...
                    self.config_manager.save()
                    self.gui_queue.put(
                        lambda c=canonical: self.lbl_subtitle.setText(f"{c} • TDE UP 2026"))
                ...
                self.gui_queue.put(lambda l=latency, tm=data.get("test_mode", False), td=data.get("test_date", "Unknown"): (
                    self.update_net_pill(
                        f"● Connected • {l}ms", THEMES[self.current_theme]["SUCCESS"] if l < 200 else THEMES[self.current_theme]["WARNING"]),
                    self.update_test_banner(tm, td)
                ))
            except Exception:
                self.gui_queue.put(lambda: (self.update_net_pill(
                    "● Offline / Timeout", THEMES[self.current_theme]["DANGER"]), self.update_test_banner(False, "")))
            time.sleep(3)

    def update_test_banner(self, is_test_mode, test_date):
        if is_test_mode:
            self.test_banner.setText(
                f"⚠️ TEST MODE ACTIVE (OVERRIDE: {test_date})")
                            ...
                            if line:
                                ...
                                if decoded.startswith("data: "):
                                    try:
                                        ...
                                        logging.error(f"SSE JSON Error: {e}")
                    else:
                        time.sleep(backoff)
            except Exception as e:
                logging.warning(
                    f"Stream dropped. Reconnecting in {backoff}s. ({e})")
                time.sleep(backoff)
    ...
    def update_net_pill(self, text, color):
        ...
        self.net_dot.setStyleSheet(
            f"color: {color}; font-size: 16px; border: none; background: transparent;")
    ...
    def manual_scan(self, lookup_type):
        ...
        val = self.ent_phone.text().strip(
        ) if lookup_type == 'phone' else self.ent_id.text().strip()
        if not val:
            return
    ...
    if os.name == 'nt':
        try:
            my_app_id = os.environ.get(
                "EVENTHUB_TOOL_ID", "EventHub.Tool.gate_display")
