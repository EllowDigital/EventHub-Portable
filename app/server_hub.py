"""
TDE UP 2026 — Event Hub (server_hub.py) — V2.3.1 (Layout Fixed)
"""
...
def ensure_ssl_certificate(local_ip):
    ...
    if not reuse_existing:
        logging.info(f"[SSL] Generating new HTTPS certificate for {local_ip}...")
...
def check_mobile():
    ...
    try:
        ...
        if existing_main:
            return jsonify({"status": "already_registered", "attendee_id": existing_main.attendee_id}), 200

        existing_kiosk = session.query(OfflineKioskAttendee).filter_by(mobile=mobile_number).first()
        if existing_kiosk:
            return jsonify({"status": "already_registered", "attendee_id": existing_kiosk.attendee_id}), 200
...
class WaitressHttpThread(threading.Thread):
    ...
    def run(self):
        try: self.server.run()
        except Exception: logging.exception("Waitress HTTP engine crashed")
...
class HttpsFlaskThread(threading.Thread):
    def __init__(self, app, host, port, numthreads=60):
        ...
        if cheroot_wsgi is None:
            raise RuntimeError("The 'cheroot' package is required.")
    ...
    def run(self):
        ...
        except Exception: logging.exception("Cheroot HTTPS engine crashed")
...
class ServerHub(ttk.Window):
    def __init__(self):
        super().__init__(themename="darkly", title="TDE UP 2026 — Event Hub V2.3.1 (Hardened + Responsive UI)")
    ...
    def build_ui(self):
        ...
        # --- SIDEBAR ---
        # FIXED: Removed pack_propagate(False) so the frame can expand vertically
        # and not cut off the "Simulator Engine" section on smaller screens.
        sidebar = ttk.Frame(self.main_frame, padding=18)
        sidebar.pack(side=LEFT, fill=BOTH)

        # Invisible spacer to ensure a minimum width of 300 without clipping height
        ttk.Frame(sidebar, width=310, height=0).pack()
    ...
    def _create_log_box(self, parent, title):
        ...
        text_widget.tag_configure("log_default", foreground="#D4D4D4")                                    
        text_widget.tag_configure("log_dim",     foreground="#6A7178")                                    
        text_widget.tag_configure("log_success", foreground="#4CD37E")                                    
        text_widget.tag_configure("log_warning", foreground="#FFB454")                                    
        text_widget.tag_configure("log_error",   foreground="#FF6B6B")                                    
        text_widget.tag_configure("log_info",    foreground="#5DADE2")                                    
        text_widget.tag_configure("log_register",foreground="#6EC6FF", font=("Consolas", 10, "bold"))     
        text_widget.tag_configure("log_checkin", foreground="#C792EA", font=("Consolas", 10, "bold"))     
        return log_box
